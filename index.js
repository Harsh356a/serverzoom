...
io.on('connection', (socket) => {
  ...
  socket.on('BE-join-room', ({ roomId, userName }) => {
    ...
    const users = Array.from(rooms[roomId]).map(user => ({
      userId: Object.keys(socketList).find(id => socketList[id].userName === user) || null,
      info: {
        userName: user,
        video: socketList[Object.keys(socketList).find(id => socketList[id].userName === user)]?.video || true,
        audio: socketList[Object.keys(socketList).find(id => socketList[id].userName === user)]?.audio || true
      }
    }));
